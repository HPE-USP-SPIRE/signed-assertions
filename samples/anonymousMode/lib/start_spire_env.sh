...
start_spire_server () {
    ...
    sleep 1
    2>/dev/null 1>&2 spire-server run -config /opt/spire/conf/server/server.conf & 
    sleep 2
}
start_spire_server
...
start_spire_agent () {
    ...
    sleep 1
    2>/dev/null 1>&2 spire-agent run -joinToken $token -config /opt/spire/conf/agent/agent.conf &
    sleep 1
    token=''
}
start_spire_agent
...
spire-server entry create \
    -parentID spiffe://example.org/host \
    -spiffeID spiffe://example.org/fakesubjectwl \
    -selector docker:label:type:fakesubjectwl
...
spire-server entry create \
    ...
    spire-server entry create \
    -parentID spiffe://example.org/host \
    -spiffeID spiffe://example.org/localtester \
    -selector unix:user:spire